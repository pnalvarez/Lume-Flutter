...
abstract class VectorList<T extends Vector> {
  ...
  VectorList(int length, int vectorLength, [int offset = 0, int stride = 0])
    : _vectorLength = vectorLength,
      ...
      _length = length,
      _buffer = Float32List(VectorList._listLength(offset, stride, vectorLength, length)) {
  ...
  VectorList.fromList(List<T> list, int vectorLength, [int offset = 0, int stride = 0])
    : _vectorLength = vectorLength,
      ...
      _stride = stride == 0 ? vectorLength : stride,
      _length = list.length,
      _buffer = Float32List(offset + list.length * (stride == 0 ? vectorLength : stride)) {
    if (_stride < _vectorLength) {
      throw ArgumentError('Stride cannot be smaller than the vector size.');
    }
  ...
  VectorList.view(Float32List buffer, int vectorLength, [int offset = 0, int stride = 0])
    : _vectorLength = vectorLength,
      ...
      _stride = stride == 0 ? vectorLength : stride,
      _length =
          (buffer.length - math.max(0, offset - stride)) ~/ (stride == 0 ? vectorLength : stride),
      _buffer = buffer {
    if (_stride < _vectorLength) {
      throw ArgumentError('Stride cannot be smaller than the vector size.');
    }
